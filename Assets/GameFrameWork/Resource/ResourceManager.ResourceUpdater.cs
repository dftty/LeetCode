            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                using(FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                {
                    ...
                    if(!zip)
                    {
                        ...
                        if(updateInfo.LoadType == LoadType.LoadFromMemoryAndQuickDecrype)
                        {
                            Utility.Encryption.GetQuickSelfXorBytes(m_ResourceManager.m_UpdateFileCache, hashBytes);
                        }else if(updateInfo.LoadType == LoadType.LoadFromMemoryAndDecript)
                        {
                    ...
                    {
                        fileStream.Close();
                        string errorMessage = Utility.Text.Format("Zip hash code error, need '{0}', downloaded '{1}'.", updateInfo.ZipHashCode.ToString("X8"), updateInfo.ZipHashCode.ToString("X8"));
                        OnDownloadFailure(this, new DownloadFailureEventArgs(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData));
                        return ;
                    }
                    ...
                    if(zip)
                    {
                        try
                        {
                            ...
                            m_ResourceManager.m_DecompressCache.Position = 0;
                            int bytesRead = 0;
                            {
